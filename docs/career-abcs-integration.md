...
| 22 | **The experience question confirms what the hub knows (C2).** `frontDoor` reads `?lvl=` (`early`/`some`/`experienced`/`leader`, inside try/catch) and maps it to `starting_out`/`early_ic`/`experienced_ic`/`people_leader`. The experience step opens with that option selected, Next enabled, and a line above the options: "From your answers on the hub: *option*. Still right? Change it if not." A stored `about.stage` wins only when `seen.survey` is already true. Nothing else in the intake changes. Also here, for F3: under 480px the app bar and the inline account bar wrap onto rows, and the stage spine labels become blocks so their ellipsis works (no sideways scroll at 390px or in the 342px hub iframe). | `src/app/onboarding.js` → `frontDoor`; F3 CSS in the app stylesheet (`.appbar-in`, `#yns-account-bar`, `.spine-t`) |
| 23 | **A changed hub level is confirmed once (N13).** New `hubLevel()` (reads and maps `?lvl=`) and `hubLevelRecheck()`. When the intake is done (`seen.survey`), a stored `about.stage` exists, the hub's stage differs from it, and `seen.hubLevel` isn't that same `lvl`, `boot` opens `frontDoor`, which shows only the experience step ("A quick check", no step dots, no Back) with the hub's answer selected and the "From your answers on the hub…" line. "That's right" runs the intake's own commit (`derivePersona` → `about.persona`, `about.level`, `build.template`) and stores `seen.hubLevel`. "Keep my earlier answer" and Escape keep the stored stage and also store `seen.hubLevel`, so the same hub answer never asks twice. The first-run intake stores `seen.hubLevel` too. Stories, resume and practice data are untouched. | `src/app/onboarding.js` → `frontDoor`, new `hubLevel`/`hubLevelRecheck`; `src/app/main.js` → `boot` |
| 24 | **Escape inside the hub closes the app (N9).** An inline script before the review-build bridge: when framed, Escape posts `{ yns: "close" }` to the parent (same message as "Back to your hub"). A window capture listener notes whether a dialog was open (`[role=dialog]`, `[aria-modal]`, `.modal`, `.modal-scrim`, `.yns-scrim`, `.front`) before the page's own handlers run; if one was, or the event was `defaultPrevented`, nothing is posted. The same block is in the three quiz prototypes. | app shell HTML template (outside the bundle) |
| 25 | **Copy (v22).** Model answers (`QUESTIONS.MODEL_ANSWERS`) each carry a `name` (Jasmine, Carmen, Leticia, Keisha, Ana, Monique, Hana, Nadia, Sofia), and each `whatChanged` opens with "*Name*'s strong version…". Nothing renders these yet; a future renderer should show the name. The coach resting message no longer says the coach is free for everyone; the coach info panel and footer no longer say there is no account. "genuinely"/"straightforward" filler removed; "practise" → "practice"; "recognised" → "recognized"; three "rather than" lines in `COACH` praise and the gap summary rewritten. | `src/content/questions.js`; `src/shell/ai.js` → `restingMessage`; `src/app/main.js` → `aiInfo`, `paintShell`, `nudgeBody`, `renderKit`; `src/app/onboarding.js`; `src/app/c-carry.js` (practice copy, `COACH` praise, gap summary); `src/content/personas.js` and `src/content/questions.js` ("genuinely"/"straightforward"); the page `<meta name="description">` |
